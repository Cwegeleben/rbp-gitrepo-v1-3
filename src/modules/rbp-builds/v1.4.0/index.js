...
// rbp-builds v1.4.0 — Build Panel with Undo/Redo v1 (Storefront module)
// Contract unchanged from v1.3.0:
// - default export: function mount(rootEl, { ctx, navigate }?)
// - Emits/custom events (unchanged):
//    • 'rbp:active-build' => detail: { id }
//    • 'rbp:build-updated' => detail: { id }
//    • 'rbp:start-part-selection' => detail: { type, slotId }
// Undo/Redo v1:
// - History per active build (bounded to 30)
// - Buttons in header and keyboard shortcuts: Ctrl/Cmd+Z (undo), Ctrl/Cmd+Shift+Z or Ctrl/Cmd+Y (redo)
// - Optimistic: apply local op -> PATCH/POST -> on failure apply inverse and toast
  ...
  // --- History ops (lightweight JS equivalent for runtime) ---
  ...
  function makeHistory(){
    return { past: [], future: [], lastTs: 0, lastType: '', push(op){
      const now = Date.now();
      // squash quick renames
      if (op.type==='RENAME_SLOT' && this.past.length>0){
        const last = this.past[this.past.length-1];
        if (last.type==='RENAME_SLOT' && last.slotId===op.slotId && now - this.lastTs <= 500){
          last.nextName = op.nextName; this.lastTs = now; this.future = []; return;
        }
      }
      this.past.push(op); this.lastTs = now; this.lastType = op.type; if (this.past.length>MAX_STACK) this.past.shift(); this.future = []; },
      canUndo(){ return this.past.length>0; },
      canRedo(){ return this.future.length>0; },
      undo(){ if (!this.canUndo()) return null; const op = this.past.pop(); this.future.push(op); return inverseOf(op); },
      redo(){ if (!this.canRedo()) return null; const op = this.future.pop(); this.past.push(op); return op; },
      reset(){ this.past = []; this.future = []; this.lastTs = 0; this.lastType=''; },
    };
  }
  ...
  function buildToolbar(state, setState, hist){
    const bar = h('div', { style: { display:'flex', gap:'8px', alignItems:'center' } });
    const btnUndo = h('button', { 'aria-label':'Undo', title: 'Undo (Ctrl/Cmd+Z)' }, 'Undo');
    const btnRedo = h('button', { 'aria-label':'Redo', title: 'Redo (Ctrl/Cmd+Shift+Z, Ctrl/Cmd+Y)' }, 'Redo');
    function update(){ btnUndo.disabled = !hist.canUndo(); btnRedo.disabled = !hist.canRedo(); }
    update();
    async function applyAndSync(op, announce){
      const b = state.currentBuild; if (!b?.id) return; const before = (b.items||[]).slice();
      const after = applyOp(before, op);
      setState({ currentBuild: { ...b, items: after }, dirty: true });
      try {
        await API.send(`/apps/proxy/api/builds/${b.id}`, 'PATCH', { items: after });
        ...
        // also reverse history movement
        // If we were doing undo: we had moved op from past->future and applied inverse; revert that
        // Simpler: push inverse to move state back to original direction
      }
      update();
    }
    btnUndo.addEventListener('click', async () => {
      const inv = hist.undo(); if (!inv) return; await applyAndSync(inv, 'Undid last action'); ensureToaster().live.textContent = 'Undid action'; });
    btnRedo.addEventListener('click', async () => {
      const next = hist.redo(); if (!next) return; await applyAndSync(next, 'Redid last action'); ensureToaster().live.textContent = 'Redid action'; });
  ...
  function renderReadinessHeader(state){
    ...
    function update(){
      const r = state.readiness; if (!r) return; const live = ensureToaster().live;
  ...
  // --- Share helpers (copied from v1.3.0) ---
  ...
  function renderSlotBadgeFor(hints){
    if (!hints || !hints.length) return null; const worst = hints.find(h=>h.severity==='error') || hints.find(h=>h.severity==='warn') || hints[0]; const color = worst.severity==='error' ? '#fee2e2' : worst.severity==='warn' ? '#fff7ed' : '#eff6ff'; const txt = worst.label; return h('span', { class: 'rbp-badge', style: { marginLeft: '6px', padding: '2px 6px', borderRadius: '999px', background: color, border: '1px solid #ddd', fontSize: '12px' } }, txt);
  }
  ...
  function renderBuildActions(state, setState, hist){
    ...
    btnClear.addEventListener('click', async () => {
      ...
      const op = { type:'CLEAR_BUILD', prevSnapshot: { id: b.id, items: before } };
      hist.push(op);
      const after = applyOp(before, op);
      setState({ currentBuild: { ...b, items: after }, dirty: true });
      try {
  ...
  function openShareModal(state, setState){
    ...
    function update(){
      if (cur?.token) {
        ...
        // @ts-ignore
        input.value = url;
        const exp = cur.expiresAt ? new Date(cur.expiresAt) : null; const leftMs = exp ? exp.getTime() - Date.now() : 0; const days = Math.max(0, Math.ceil(leftMs/(24*60*60*1000)));
        ...
        copy.disabled = true; revoke.disabled = true; openPrev.disabled = true; regen.disabled = false;
      }
    }
    ...
    regen.addEventListener('click', async () => {
      const id = state.currentBuild?.id; if (!id) return; regen.disabled = true; try { const next = await mintShareToken(id); setShare(next); toast('success', 'New link generated'); } catch { toast('error','Failed to generate link'); } finally { regen.disabled = false; }
    });
    revoke.addEventListener('click', async () => { setShare(null); toast('success', 'Link revoked for this session'); });
    openPrev.addEventListener('click', () => { if (!cur?.token) return; window.open(buildShareUrl(cur.token), '_blank', 'noopener'); });

    (async () => { if (!state.currentBuild?.id) { update(); return; } try { const minted = await mintShareToken(state.currentBuild.id); setShare(minted); } catch { setShare(null); } })();
  }

  function renderSlots(state, setState, hist){
      ...
      wrap.appendChild(h('div', { class:'rbp-group', style: { marginTop: '10px', paddingTop: '6px', borderTop: '1px solid #eee' } }, [
        ...
        ...items.map((it, idx) => {
          ...
          name.addEventListener('blur', async () => {
            const prevName = it.label || it.name || '';
            const newItems = (b.items||[]).map(x => x===it ? { ...x, label: name.value } : x);
            const prev = b.items||[]; const op = { type:'RENAME_SLOT', slotId: it.id||it.slotId||`${type}-${idx}`, prevName, nextName: name.value };
            hist.push(op);
            setState({ currentBuild: { ...b, items: newItems }, dirty: true });
    ...
    function move(item, delta){
      ...
      const op = { type:'REORDER_SLOT', slotId: (item.id||item.slotId||''), from: i, to: j };
      hist.push(op);
    ...
    function del(item){
      const before = (b.items||[]).slice();
      const snap = item; const op = { type:'REMOVE_SLOT', slotSnapshot: snap };
  ...
  function mount(root, _props){
    ...
    const container = h('div', { class: 'rbp-builds-v214', style: { fontFamily: 'system-ui,-apple-system,Segoe UI,Roboto,sans-serif' } });
    root.appendChild(container);

  const state = { builds: [], activeId: null, currentBuild: null, dirty: false, readiness: null, share: null };
    function setState(patch){ Object.assign(state, patch||{}); render(); }

    const historyByBuild = new Map();
    function getHist(){ const id = state.activeId||'@none'; if (!historyByBuild.has(id)) historyByBuild.set(id, makeHistory()); return historyByBuild.get(id); }
    ...
    function render(){
      container.innerHTML = '';
      const hist = getHist();
      const header = h('div', { style: { display:'flex', alignItems:'center', justifyContent:'space-between', gap:'8px' } });
      const saved = renderSavedBuildsBar(state, setState); header.appendChild(saved);
      const toolbar = buildToolbar(state, setState, hist); header.appendChild(toolbar);
      ...
      container.appendChild(renderBuildActions(state, setState, hist));
      container.appendChild(renderSlots(state, setState, hist));
    }

    function onKey(e){
      // Don’t intercept when an input/textarea or contenteditable has focus
      ...
      if (e.key.toLowerCase()==='z' && !e.shiftKey){
        const hist = getHist(); if (!hist.canUndo()) return; e.preventDefault(); const inv = hist.undo(); if (!inv) return;
        const b = state.currentBuild; if (!b?.id) return; const before = (b.items||[]).slice(); const after = applyOp(before, inv);
        setState({ currentBuild: { ...b, items: after }, dirty: true });
        API.send(`/apps/proxy/api/builds/${b.id}`, 'PATCH', { items: after }).then(()=>{ toast('success','Undid action'); window.dispatchEvent(new CustomEvent('rbp:build-updated', { detail: { id: b.id } })); }).catch(()=>{ setState({ currentBuild: { ...b, items: before }, dirty: false }); toast('error','Undo failed — rolled back'); });
      } else if ((e.key.toLowerCase()==='z' && e.shiftKey) || e.key.toLowerCase()==='y'){
        const hist = getHist(); if (!hist.canRedo()) return; e.preventDefault(); const next = hist.redo(); if (!next) return;
        const b = state.currentBuild; if (!b?.id) return; const before = (b.items||[]).slice(); const after = applyOp(before, next);
        setState({ currentBuild: { ...b, items: after }, dirty: true });
        API.send(`/apps/proxy/api/builds/${b.id}`, 'PATCH', { items: after }).then(()=>{ toast('success','Redid action'); window.dispatchEvent(new CustomEvent('rbp:build-updated', { detail: { id: b.id } })); }).catch(()=>{ setState({ currentBuild: { ...b, items: before }, dirty: false }); toast('error','Redo failed — rolled back'); });
      }
    }
    window.addEventListener('keydown', onKey);

    window.addEventListener('rbp:active-build', async (e) => { const id = e?.detail?.id; setState({ activeId: id }); const hist = getHist(); hist.reset(); scheduleDryRun(); });
    ...
    return { unmount(){ try { root.innerHTML = ''; } catch {} if (debounceTimer) clearTimeout(debounceTimer); if (inFlight) inFlight.abort(); window.removeEventListener('keydown', onKey); } };
  }
  // @ts-ignore expose for manual testing
  if (typeof window !== 'undefined') window.RBP_BUILDS_V214 = { mount };
