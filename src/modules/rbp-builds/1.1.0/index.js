...
// rbp-builds 1.1.0 — Build Selector Panel v2 (UI-only, DOM implementation)
// Contract:
// - default export: function mount(rootEl, { ctx, navigate }?)
// - Renders Saved Builds bar + actions + slots grouping
// - Uses /apps/proxy/api/builds endpoints (GET list, GET by id, POST create, PATCH update, DELETE item not used)
// - Emits/custom events:
//    • 'rbp:active-build' => detail: { id }
//    • 'rbp:build-updated' => detail: { id }
// - a11y: aria-live region; keyboard nav on saved builds bar (ArrowLeft/Right)
  ...
  function toast(kind, text){
    ...
    setTimeout(()=>{ if (card.parentElement===host) host.removeChild(card); }, 3200);
  }
  ...
  function groupItems(items){
    const g = new Map();
    (items||[]).forEach(it => {
      const t = it.type || it.slotType || 'Other';
      if (!g.has(t)) g.set(t, []);
      g.get(t).push(it);
    });
    return g; // Map<type, items[]>
  }
      ...
      wrap.appendChild(h('div', { class:'rbp-group', style: { marginTop: '10px', paddingTop: '6px', borderTop: '1px solid #eee' } }, [
        ...
        ...items.map((it, idx) => {
          ...
          name.addEventListener('blur', async () => {
            ...
            try { await API.send(`/apps/proxy/api/builds/${b.id}`, 'PATCH', { items: newItems }); toast('success','Renamed'); }
          ...
          // Select part
          const sel = h('button', { 'aria-label': 'Select part' }, 'Select');
          ...
          row.appendChild(name); row.appendChild(up); row.appendChild(down); row.appendChild(remove); row.appendChild(sel);
          return row;
        })
      ]));
    }
  ...
  function mount(root, _props){
    ...
    const container = h('div', { class: 'rbp-builds-v2', style: { fontFamily: 'system-ui,-apple-system,Segoe UI,Roboto,sans-serif' } });
    root.appendChild(container);

    const state = { builds: [], activeId: null, currentBuild: null, dirty: false };
    ...
    window.addEventListener('rbp:active-build', (e) => { const id = e?.detail?.id; if (id) setState({ activeId: id }); });
    window.addEventListener('rbp:build-updated', (e) => { const id = e?.detail?.id; if (!id || id!==state.activeId) return; refreshBuilds(state, setState); });

    refreshBuilds(state, setState);
    return { unmount(){ try { root.innerHTML = ''; } catch {} } };
  }
  // expose in browser for manual testing
  // @ts-ignore
  if (typeof window !== 'undefined') window.RBP_BUILDS_V2 = { mount };
