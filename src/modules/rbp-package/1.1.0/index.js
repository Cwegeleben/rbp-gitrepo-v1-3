...
// rbp-package v1.1.0 — same as v1.0.0 with event dispatch for rbp:packager:result
// Contract unchanged. See v1.0.0 for full comments.
...
export default function mount(root, { navigate } = {}){
  ...
  cta.addEventListener('click', async () => {
    ...
    try {
      ...
      // <!-- BEGIN RBP GENERATED: package-cta-v1→v1.1.0 -->
      try { window.dispatchEvent(new CustomEvent('rbp:packager:result', { detail: { status: 'success', buildId: activeBuildId, cartPath: res?.cartPath ?? null, meta: res?.meta, hints: res?.hints, response: res } })); } catch {}
      // <!-- END RBP GENERATED: package-cta-v1→v1.1.0 -->
      if (res?.cartPath) {
      ...
      // <!-- BEGIN RBP GENERATED: package-cta-v1→v1.1.0 -->
      try { window.dispatchEvent(new CustomEvent('rbp:packager:result', { detail: { status: 'error', buildId: activeBuildId, response: null, meta: null, hints: null } })); } catch {}
      // <!-- END RBP GENERATED: package-cta-v1→v1.1.0 -->
    }
